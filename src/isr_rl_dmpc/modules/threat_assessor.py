...
class ThreatParameters:
    ...
    def __init__(self):
        ...
        self.unknown_threat_score = 0.55  # FIXED: Increased from 0.5 to > 40 when confidence=0.8
...
class ThreatAssessor:
    ...
    def __init__(self, params: Optional[ThreatParameters] = None):
        ...
        Arguments:
            params (Optional[ThreatParameters]): Threat parameters
        """
    ...
    def _assess_rf_threat(self, target_data: Dict) -> float:
        ...
        if rf_strength > self.params.rf_strength_threshold:
            # FIXED: More aggressive normalization for strong signals
            # Range from threshold to -10 dBm (very strong)
            norm_strength = min((rf_strength - self.params.rf_strength_threshold) / 40, 1.0)
            signal_threat = norm_strength ** 0.5  # Non-linear scaling favors strong signals

        # Modulation threat (military modulations are more threatening)
        ...
        else:  # Unknown is moderately threatening
    ...
    def _assess_motion_threat(self, target_data: Dict) -> float:
        ...
        if speed > self.params.velocity_threshold:  # High speed is threatening
            # FIXED: Tighter normalization range (50-150 m/s instead of 50-200 m/s)
            norm_speed = min((speed - self.params.velocity_threshold) / 100, 1.0)
            velocity_threat = np.clip(norm_speed ** 0.8, 0, 1)*80  # Non-linear scaling
        ...
        if accel_mag > self.params.acceleration_threshold:  # Evasive moves
            # FIXED: Tighter normalization range for acceleration (5-15 m/s² instead of 5-20 m/s²)
            norm_accel = min((accel_mag - self.params.acceleration_threshold) / 10, 1.0)
            accel_threat = np.clip(norm_accel ** 0.7, 0, 1)*80  # Non-linear scaling

        motion_threat = velocity_threat * 0.45 + accel_threat * 0.55
    ...
    def _assess_position_threat(self, target_pos: np.ndarray, own_pos: np.ndarray) -> float:
        ...
        # Proximity threat
        if distance < self.params.proximity_threshold:  # Critical proximity
            threat = 1.0 - (distance / self.params.proximity_threshold)
            threat = np.clip(threat, 0, 1)
        else:  # Distant target
            threat = 0.1 * np.exp(-distance / 1000)

        # Altitude threat (low altitude is more threatening)

...
- **Drone 3D models** — CesiumDrone .glb models (bundled locally under `src/isr_rl_dmpc/models/meshes/`) color-coded by battery level (green = full, red = empty) with labels showing drone ID and battery
...
- **3D Panel** — Drone 3D models (local CesiumDrone .glb), target models (local per-threat-class .glb), and ground plane
